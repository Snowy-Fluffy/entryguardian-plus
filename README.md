...
| `/delete_all [target] [c<N>\|period]` | moderators, admins, owners | Bulk-delete a user's **tracked messages in this chat** — by reply or `@username`/ID, same target resolution as `/mute`/`/ban`. With no extra argument, deletes everything tracked (up to the 2-day retention window below); `c200` deletes their last 200 tracked messages; a duration like `1h`/`1d` deletes everything from that period onward (rejected with an error if it exceeds the 2-day retention window). e.g. `/delete_all @spammer 1h` or reply with `/delete_all c50`. Confirms in the chat with how many were deleted and whether it was "all", a count, or a period. **Reply to a channel's post** with `/delete_all` to bulk-delete that channel's tracked messages instead (same as banning a channel, this only works by reply, not by ID/@username) |
| `/sdelete_all` | admins, owners | Silent variant of `/delete_all` — same bulk deletion, nothing posted to the chat |
...
**Message tracking for `/delete_all`:** the bot keeps a rolling 2-day log of `(chat, user, message id, timestamp)` for every group message it sees, and separately `(chat, channel, message id, timestamp)` for messages posted as a channel (channel posts otherwise all share one anonymous `@Channel_Bot` sender, so they're tracked by the real channel id instead) — no message content, just enough to locate and delete them later, auto-expired in the background. This backs `/delete_all`/`/sdelete_all` — deleting a raider's (or spam channel's) messages in bulk — and is capped at 2 days because that's also the limit of how old a message Telegram lets a bot delete.
