...
1. A new user joins the group → bot **restricts them first**, records them as pending, and only then posts a welcome message with a button linking to the bot's DM (so a flood error on the message can never leave a newcomer unrestricted). Other bots are ignored. Only the most recent welcome message stays up in a chat — a join wave doesn't pile them up; earlier joiners still reach the captcha through `/start`. A member who is still pending and somehow able to post (a restriction lifted out-of-band, an expired timed mute, a restart) has the message deleted and the restriction re-applied. A newcomer who joins while muted (locally or globally) is muted *and* still put through the captcha.
...
Sessions expire after `CAPTCHA_TIMEOUT` (10 minutes by default; the bot's message says so). The code uses an alphabet without look-alike glyphs (no `0/O`, `1/I/L`, `5/S`, `8/B`), and only a message that *looks like* a code (6 letters/digits) counts as an attempt — a sticker or a "hi" in DM just gets a reminder to send the code. Failed code attempts are limited; too many wrong attempts result in a temporary block. If the web part is already done and a code was issued, `/start` reminds the user to type it instead of starting a new game. A newcomer who never passes the captcha within **24 hours** is kicked from the chat (and immediately unbanned, so they can rejoin and try again). This 24h kick is on by default and can be turned off per chat from the admin panel; the welcome message tells the newcomer about the 24-hour limit only when the kick is enabled for that chat. The unban half of a kick is recorded in a persistent queue and retried (honouring rate-limit `Retry-After`) until it succeeds, so a failed/rate-limited unban — or a restart mid-kick — never leaves someone stuck banned. Re-joining within an hour of the last prompt doesn't re-post the welcome message, to avoid spam.
...
NOTIFY_OWNERS_GBAN=0        # DM every owner (except the issuer) about each new global ban: who, by whom, from where, reason
```
...
| `/mute [duration] [reason]` | moderators, admins, owners | Mute the target in **this** chat. Duration like `10m` (minutes), `1h`, `1d`, `1w`, `2mo` (months — note `mo`, since `m` is minutes); **minimum 1 minute** (seconds are refused with a hint); omit for permanent; reason optional (reasons are capped at 300 characters). e.g. `mute @user 10m spam` or reply with `mute 1d` |
...
| `/broadcast` | owners only, **private chat with the bot only** (silently ignored elsewhere) | Mass-send a message. Pick the audience with a button — **everyone**, **chats only**, or **DMs only** — then send the content (formatted text, photo, video, document...). The bot echoes it back as a preview, shows the recipient counts and asks for confirmation; recipients get an exact copy (no "forwarded" header). See below for who counts as a recipient |
...
**Telegram-native admins can't be punished:** if the target of `/ban`, `/sban`, `/mute` or `/smute` is an administrator or the creator of the chat *as appointed through Telegram itself* (regardless of any bot role), the bot refuses with "Can't punish a chat administrator appointed through Telegram" — Telegram would reject the ban/mute anyway, and refusing up front avoids a blocklist/mute record the chat couldn't enforce. The **global** variants (`/gban`, `/sgban`, `/gmute`, `/gsmute`) check *every* chat the bot knows, from a group or from DM, and refuse naming the chat where the target is a native admin — otherwise the ban would fail there while the bot kept deleting that admin's messages.

**Telegram's service accounts can't be targeted.** A post made *as a channel* and a post by an *anonymous admin* both come from a shared pseudo-account (`@Channel_Bot` / `GroupAnonymousBot`); punishing that id would hit every channel post or every anonymous admin everywhere, so every command refuses it with an explanation (ban a channel by replying to its post). Likewise an `@username` or negative id that resolves to a channel/group is refused for user commands. Any such rows written by earlier versions are removed on startup.
...
**Broadcast audience.** "Chats" are every group the bot has observed (see the note on chat discovery below) except those an owner stopped it in with `/stopchat`. "DMs" are users known to have a private chat with the bot — anyone who has ever written to it in DM (`/start`, a captcha code, the admin panel...), plus everyone who ever went through the captcha — minus the global blocklist and the sender. A bot cannot open a conversation on its own, so a user who never wrote to it is unreachable and is simply not counted. Delivery is paced (~20 messages/s, backing off on Telegram's `Retry-After`), progress is shown on the confirmation message, and the final line reports delivered/failed counts; a user who has blocked the bot is dropped from the DM audience on first failure. Only one broadcast can run at a time.
...
The **global** commands (`/gban`, `/sgban`, `/gmute`, `/gsmute`, `/ungban`, `/unsgban`, `/ungmute`, `/ungsmute`) can also be sent in a **private chat with the bot** — by **owners only** (target by ID or @username). Chat admins can issue them only from inside a group they administer; from DM the bot answers "no permission". When issued from DM the broadcast to all chats omits the source chat — it just says "globally banned/muted/…" — and the issuer gets a confirmation in DM. **Every global action is logged in every chat** the bot knows (the staff action log in `/admin`, and therefore `/punl` in any of them) — whether issued from a group or from DM, by an admin or an owner. `/punl` in DM aggregates across chats and collapses those repeated entries into one. Silent variants still post and DM nothing to the chats/target.

**Owner notifications for global bans** (`NOTIFY_OWNERS_GBAN=1`): every owner except the one who issued it gets a DM for each new `/gban`, `/sgban` or global channel ban — who was banned, by whom, from which chat (or "private chat with the bot" for DM-issued ones) and the reason. Silent variants are reported too — "silent" is about the chats, not the owners.

**Local beats global.** A local decision in a chat overrides a global one there, for bans and mutes alike: `/unban` of a globally banned user, or `/unmute` of a globally muted one, lifts the punishment in that chat only (recorded as a per-chat exception); a later local `/ban`/`/mute` in the same chat cancels that exception; a fresh global `/gban`/`/gmute` (or its reversal) resets every local exception for that user. In the other direction, a global **amnesty** (`/ungban`, `/ungmute`) lifts only the global punishment: a chat's own local `/ban` or `/mute` stays in force there — an admin of chat B can't undo chat A's local decisions. Everything — local and global mutes/bans and their exceptions — is stored in the database and enforced from it, so it survives restarts.
...
- toggle **"block invited bots"** for that chat (off by default). When on, any bot added to the chat by an ordinary member is banned immediately, with a notice naming the bot and who added it, and a staff-log entry (also visible in that bot's `/punl`). A bot added by a Telegram-native admin or by the chat's bot staff (admin/moderator/owner) is left alone — that's assumed deliberate. Bots already in the chat are not touched (the Bot API can't list members);
...
- view the **global ban list** — a paged, searchable list of every blocklisted user and channel (those banned via `/gban` / `/sgban`), each with the date and time the ban was issued (for bans predating this version the date is recovered from the staff log where possible, otherwise shown as "date unknown"), same 🔍 search UI as the staff action log (by id, username, or cached display name for users; id or title for channels). Removal is still done with `/ungban`.
...
**Reactions** get the same treatment as messages, since a reaction is the one thing a restricted member can still do (a still-unverified member who reacts is removed from the chat for `COOL_DOWN` — at least a minute — and told why in DM): a reaction from a blocklisted user bans them in that chat; a reaction left *as a channel* bans the channel if it is on the global channel blocklist or if the chat has **"Channels: forbidden"** turned on (with the same "Channels are forbidden in this chat" notice as for a channel post); an anonymous admin's reaction (left as the group itself) is ignored. The reaction itself can't be removed — the Bot API has no way to take back someone else's reaction — so the ban is the whole response. Reacting also registers the user in the bot's id↔username cache and the chat in its list of known chats, same as a message would — and, if the bot had never seen the user before, records that chat as where they first showed up (the "Came in via chat" line in `/uinfo`/`/punl`, marked "(via a reaction)"), exactly as a group join would. The same happens the first time the bot sees someone *post* in a group ("(via a message)") — so members who were in a chat before the bot get an origin too. Whichever of join / reaction / message the bot sees first is what's recorded, and it's never overwritten.
...
```bash
mkdir -p data          # the SQLite database (and its WAL/journal files) live here
docker compose up -d
docker compose logs -f
```

The database is stored as `./data/users.db` (`docker-compose.yml` mounts the `./data` directory and sets `DB_PATH` accordingly — a directory rather than a single file, because SQLite keeps its write-ahead log next to the database and a single-file mount would leave that inside the container). **Upgrading from a version that mounted `./users.db` directly:** stop the bot, `mkdir -p data && mv users.db data/`, then start it again.

The bot logs to stdout (`docker compose logs -f`): startup, every crash of a background task (which is restarted automatically instead of taking the bot down), and errors from spawned work.
